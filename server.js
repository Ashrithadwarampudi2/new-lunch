...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM dbo.lunch_orders ORDER BY submitted_at DESC');
    res.json(result.recordset);
  } catch (err) {
...
app.get('/api/restaurants', async (req, res) => {
  ...
  try {
    ...
    console.log(`[restaurants] query completed, rows=${result.recordset ? result.recordset.length : 0}`);
    res.json(result.recordset);
  } catch (err) {
    ...
    res.status(500).json({ error: 'Database error fetching restaurants.' });
  }
});
